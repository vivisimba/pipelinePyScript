...
def mvOrCpFileExcludeJobserver(codeDir, moduleName):
    ...
    if moduleName in extraWarList:
        # 移动war包（war包与模块名称不一致）
        warSrcPath = findFile(moduleDir, CONFIG.EXTRA_WAR_DIC[moduleName] + ".war")
        warDesPath = os.path.join(codeDir, "outputFileFolder", CONFIG.EXTRA_WAR_DIC[moduleName] + ".war")
        shutil.move(warSrcPath, warDesPath)
        
    elif moduleName in CONFIG.MV_ZIP_LIST:
        # 移动zip包

        zipSrcPath = findFile(moduleDir, moduleName + ".zip")
        zipDesPath = os.path.join(codeDir, "outputFileFolder", moduleName + ".zip")
        shutil.move(zipSrcPath, zipDesPath)
...
def mvOrCpFile(codeDir, moduleName):
    ...
    if moduleName == "jobserver":
        ...
        for jarKey in CONFIG.JAR_FORJOBSERVER_RELATIVEPATH_DIC.keys():
            if jarKey == "stariboss-billing.jar":
                jarSrcPath = os.path.join(codeDir, CONFIG.JAR_FORJOBSERVER_RELATIVEPATH_DIC[jarKey])
                jarDesPath = os.path.join(wholeJarfilefolder, "billing.jar")
            elif jarKey == "stariboss-order-job.jar":
                jarSrcPath = os.path.join(codeDir, CONFIG.JAR_FORJOBSERVER_RELATIVEPATH_DIC[jarKey])
                jarDesPath = os.path.join(wholeJarfilefolder, "order-job.jar")
            else:
                jarSrcPath = os.path.join(codeDir, CONFIG.JAR_FORJOBSERVER_RELATIVEPATH_DIC[jarKey])
                jarDesPath = os.path.join(wholeJarfilefolder, jarKey)
            shutil.move(jarSrcPath, jarDesPath)
            
        # 移动jobserver的dockerfile
        ...
        shutil.move(jobserverFileSrcPath, jobserverFileDesPath)
        ...
        shutil.move(uploadJarFileSrcPath, uploadJarFileDesPath)
        ...
        shutil.move(uploadShellSrcPath, uploadShellDesPath)
